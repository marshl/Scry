...
public class ScryApplication
{
	...
	public void AddCard( OracleCard _card ) throws Exception
	{
		if ( cardMap.containsKey( _card.id ) )
		{
			throw new Exception( "Card ID " + _card.id + " already set to "
		         + cardMap.get( _card.id ) + " when adding " + _card.name );
		}
	...
	public void Search()
	{
		...
		searchAllTypes = searchTypeCreature && searchTypeArtifact && searchTypeEnchantment
			&& searchTypeInstant && searchTypeSorcery && searchTypeLand && searchTypePlaneswalker;
